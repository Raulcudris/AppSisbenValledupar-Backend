...
public class VentanillaService {
    ...
    private static final Logger log =
            LoggerFactory.getLogger(VentanillaService.class);
    ...
    public PageResponse<VentanillaResponse> findAll(
            Pageable pageable
    ) {
        Page<VentanillaRegistro> page = isAdmin()
                ? repository.findAll(pageable)
                : repository.findAll(
                VentanillaSpecification.activeOnly(),
                pageable
        );

        List<VentanillaResponse> content =
    ...
    @Transactional(readOnly = true)
    public PageResponse<VentanillaResponse> search(
        ...
        Page<VentanillaRegistro> page = repository.findAll(
                VentanillaSpecification.byFilter(
                        filter,
    ...
    public VentanillaResponse findById(Long id) {
        VentanillaRegistro entity = isAdmin()
                ? findEntity(id)
                : findActiveEntity(id);
    ...
    @Transactional(readOnly = true)
    public PageResponse<VentanillaUserHistorySummaryResponse>
    findUserHistorySummaries(
        ...
        Page<VentanillaUserHistorySummaryResponse> page =
                repository.findUserHistorySummaries(
                        normalizedSearch,
                        pageable
                );
    ...
    @Transactional(readOnly = true)
    public VentanillaUserHistoryResponse findUserHistory(
            String cedulaUsuario
    ) {
        if (isBlank(cedulaUsuario)) {
            throw new BusinessException(
                    "La cédula del ciudadano es obligatoria"
            );
        }

        String cedula = cedulaUsuario.trim();

        List<VentanillaRegistro> records =
                repository.findTraceabilityByCedula(cedula);
        ...
        List<VentanillaRegistro> orderedRecords = records
                ...
                .sorted(
                        Comparator
                                .comparing(VentanillaRegistro::getFecha)
                                .reversed()
                                .thenComparing(
                                        VentanillaRegistro::getId,
    ...
    public byte[] exportUserHistory(String cedulaUsuario) {
        ...
        csv.append(
                "Fecha,N° Ventanilla,Cédula,Nombre usuario,Teléfono,"
                        + "Solicitud,Categoría,Estado solicitud,Barrio,Comuna,"
                        + "Funcionario,Extranjero,Estado registro,"
                        + "Motivo repetición,Observación\n"
        );

        for (VentanillaUserHistoryItemResponse item :
                history.solicitudes()) {
            ...
            csv.append(csvValue(item.fecha()))
                    ...
                    .append(
                            csvValue(
                                    Boolean.TRUE.equals(item.extranjero())
                                            ? "Sí"
                                            : "No"
                            )
                    )
    ...
    @Transactional(readOnly = true)
    public byte[] exportUserHistoryPdf(
            String cedulaUsuario
    ) {
        VentanillaUserHistoryResponse history =
                findUserHistory(cedulaUsuario);
        ...
        try {
            ...
            addUserHistoryPdfHeader(
                    document,
                    history
            );

            addUserHistoryPdfSummary(
                    document,
                    history
            );

            addUserHistoryPdfTable(
                    document,
                    history
            );
            ...
            log.error(
                    "Error generando PDF de historial de usuario. Cedula={}",
                    cedulaUsuario,
                    exception
            );
    ...
    public VentanillaDailyValidationResponse validateBeforeSave(
        ...
        DailyRequestValidation validation =
                evaluateDailyRequests(
                        currentId,
                        fecha,
                        cedulaUsuario,
                        solicitudId
        ...
        List<VentanillaDailyRequestItemResponse> items =
                validation.dailyRequests()
                        .stream()
                        .map(this::toDailyRequestItem)
                        .toList();

        if (validation.duplicatedRequest()) {
            return new VentanillaDailyValidationResponse(
                    "SOLICITUD_DUPLICADA_MISMA_FECHA",
                    "Solicitud repetida el mismo día",
                    "Este ciudadano ya tiene registrada esta misma "
                            + "solicitud para la fecha seleccionada. "
                            + "Para guardarla nuevamente debes registrar "
                            + "el motivo.",
                    true,
                    true,
                    (long) validation.dailyRequests().size(),
                    items
            );
        }

        if (!validation.dailyRequests().isEmpty()) {
            return new VentanillaDailyValidationResponse(
                    "SOLICITUD_DIFERENTE_MISMA_FECHA",
                    "El ciudadano ya tiene solicitudes registradas hoy",
                    "Este ciudadano ya tiene solicitudes registradas "
                            + "para la fecha seleccionada. Para guardar "
                            + "una nueva solicitud debes registrar el motivo.",
                    true,
                    true,
                    (long) validation.dailyRequests().size(),
                    items
        ...
        return new VentanillaDailyValidationResponse(
                ...
                "El ciudadano no tiene solicitudes registradas "
    ...
    @Transactional(readOnly = true)
    public VentanillaCitizenHistoryResponse findCitizenHistory(
            String cedulaUsuario
    ) {
        if (isBlank(cedulaUsuario)) {
            throw new BusinessException(
    ...
    @Transactional
    public VentanillaResponse create(
            VentanillaRequest request
    ) {
        validateDailySolicitudLimit(
                null,
                request
        );

        VentanillaRegistro entity =
                new VentanillaRegistro();
    ...
    @Transactional
    public VentanillaResponse update(
            Long id,
            VentanillaRequest request
    ) {
        VentanillaRegistro entity = isAdmin()
    ...
    public void delete(Long id) {
        /*
         * Compatibilidad con el endpoint DELETE existente.
    ...
    private void updateActiveStatus(
        ...
        VentanillaRegistro entity = adminRequired
                ? findEntity(id)
                : findActiveEntity(id);
    ...
    private void validateDailySolicitudLimit(
            Long currentId,
            VentanillaRequest request
    ) {
        if (request == null) {
        ...
        if (
                !validation.dailyRequests().isEmpty()
                        && isBlank(request.motivoRepeticion())
        ) {
            throw new BusinessException(
                    validation.duplicatedRequest()
                            ? "Este ciudadano ya tiene registrada esta "
                            + "misma solicitud para la fecha seleccionada. "
                            + "Para guardarla nuevamente debes registrar "
                            + "el motivo."
                            : "Este ciudadano ya tiene solicitudes "
                            + "registradas para la fecha seleccionada. "
                            + "Para guardar una nueva solicitud debes "
                            + "registrar el motivo."
            );
        }
    }

    private DailyRequestValidation evaluateDailyRequests(
            Long currentId,
            LocalDate fecha,
            String cedulaUsuario,
            Long solicitudId
        ...
        List<VentanillaRegistro> dailyRequests =
                currentId == null
                        ? repository.findDailyRequestsByCedula(
                        fecha,
                        cedula
                )
                        : repository.findDailyRequestsByCedulaAndIdNot(
                        fecha,
                        cedula,
                        currentId
                );
        ...
        boolean duplicatedRequest =
                dailyRequests
                        .stream()
                        .anyMatch(record ->
                                record.getSolicitud() != null
                                        && record.getSolicitud().getId() != null
                                        && record.getSolicitud()
                                        .getId()
                                        .equals(solicitudId)
                        );

        return new DailyRequestValidation(
                dailyRequests,
                duplicatedRequest
        );
    }
    ...
    private void apply(
        ...
        EstadoSolicitud estado =
                findActiveEstadoSolicitud(
                        request.estadoSolicitudId()
                );
        ...
        entity.setNumeroVentanilla(
                request.numeroVentanilla().trim()
        );

        entity.setCedulaUsuario(
                request.cedulaUsuario().trim()
        );

        entity.setNombreUsuario(
        ...
        entity.setExtranjero(
                request.extranjero() != null
                        ? request.extranjero()
                        : Boolean.FALSE
        );
        ...
        entity.setMotivoRepeticion(
                normalizeOptionalText(
                        request.motivoRepeticion()
                )
        );
    }

    private Categoria findActiveCategoria(
            Long categoriaId
    ) {
    ...
    private Solicitud findActiveSolicitud(
            Long solicitudId
    ) {
        ...
        if (!Boolean.TRUE.equals(solicitud.getActivo())) {
            throw new BusinessException(
                    "La solicitud seleccionada está inactiva"
            );
        }
    ...
    private EstadoSolicitud findActiveEstadoSolicitud(
            Long estadoSolicitudId
    ) {
        EstadoSolicitud estado =
                estadoSolicitudRepository
                        .findById(estadoSolicitudId)
                        .orElseThrow(
                                () -> new ResourceNotFoundException(
                                        "Estado de solicitud no encontrado"
                                )
                        );

        if (!Boolean.TRUE.equals(estado.getActivo())) {
            throw new BusinessException(
                    "El estado de solicitud seleccionado está inactivo"
            );
        }
    ...
    private List<VentanillaResponse> toResponses(
            List<VentanillaRegistro> entities
    ) {
        if (entities == null || entities.isEmpty()) {
            return List.of();
        }

        Map<String, List<VentanillaTraceabilityRow>>
                historyByCedula =
                loadTraceabilityHistory(entities);
    ...
    private VentanillaResponse toResponse(
            VentanillaRegistro entity,
            Map<String, List<VentanillaTraceabilityRow>>
                    historyByCedula
    ) {
        return new VentanillaResponse(
                ...
                entity.getSolicitud() != null
                        ...
                        : null,
                entity.getEstadoSolicitud() != null
    ...
    private Map<String, List<VentanillaTraceabilityRow>>
    loadTraceabilityHistory(
            List<VentanillaRegistro> entities
    ) {
        List<String> cedulas = entities
                .stream()
                .map(VentanillaRegistro::getCedulaUsuario)
                .map(this::normalizeCedula)
        ...
        List<VentanillaTraceabilityRow> rows =
                repository.findTraceabilityRowsByCedulas(
                        cedulas
                );
        ...
        for (VentanillaTraceabilityRow row : rows) {
            if (row == null) {
                continue;
            }

            String normalizedCedula =
                    normalizeCedula(row.cedulaUsuario());
    ...
    private VentanillaDailyRequestItemResponse toDailyRequestItem(
            VentanillaRegistro entity
    ) {
    ...
    private VentanillaUserHistoryItemResponse toUserHistoryItem(
            VentanillaRegistro entity
    ) {
    ...
    private String neutralizeCsvFormula(String value) {
        ...
        while (
                firstVisibleCharacterIndex < value.length()
                        && Character.isWhitespace(
                        value.charAt(firstVisibleCharacterIndex)
                )
        ) {
            firstVisibleCharacterIndex++;
        }

        if (firstVisibleCharacterIndex >= value.length()) {
            return value;
        }

        char firstVisibleCharacter =
                value.charAt(firstVisibleCharacterIndex);
        ...
        /*
         * El apóstrofo obliga a Excel y aplicaciones similares
         * a interpretar el contenido como texto.
         */
    ...
    private VentanillaTraceabilityBadgeResponse
    buildTraceabilityBadge(
            VentanillaRegistro entity,
            Map<String, List<VentanillaTraceabilityRow>>
                    historyByCedula
        ...
        if (
            ...
            return unavailableTraceabilityBadge();
        }

        String normalizedCedula =
                normalizeCedula(entity.getCedulaUsuario());

        List<VentanillaTraceabilityRow> history =
                historyByCedula.getOrDefault(
                        normalizedCedula,
                        List.of()
                );

        long totalVisitas = history.size();

        LocalDate fechaInicioVentana =
                entity.getFecha().minusDays(29);

        long visitasUltimos30Dias = 0L;

        VentanillaTraceabilityRow previousVisit = null;

        for (VentanillaTraceabilityRow row : history) {
            if (
                    row == null
                            || row.fecha() == null
            ) {
            ...
            boolean insideThirtyDayWindow =
                    !row.fecha().isBefore(fechaInicioVentana)
                            && !row.fecha().isAfter(
                            entity.getFecha()
                    );

            if (insideThirtyDayWindow) {
                visitasUltimos30Dias++;
            }

            boolean previousByDate =
                    row.fecha().isBefore(entity.getFecha());

            boolean previousByIdOnSameDate =
                    row.fecha().isEqual(entity.getFecha())
                            && row.id() != null
                            && row.id().compareTo(entity.getId()) < 0;

            if (
                    previousByDate
                            || previousByIdOnSameDate
            ) {
                previousVisit = row;
            }
        }
        ...
        if (ultimaVisitaAnterior == null) {
            ...
            descripcion =
                    "No registra atenciones anteriores en Ventanilla.";
            ...
            descripcion =
                ...
                diasDesdeUltimaVisitaAnterior != null
                        && diasDesdeUltimaVisitaAnterior == 0
        ) {
            nivel = "MISMO_DIA";
            etiqueta = "Vino hoy";
            color = "info";
            ...
            descripcion =
                    "Ya tenía otra atención registrada "
                            + "el mismo día.";
    ...
    private VentanillaTraceabilityBadgeResponse
    unavailableTraceabilityBadge() {
        return new VentanillaTraceabilityBadgeResponse(
                "NO_DISPONIBLE",
                "Sin trazabilidad",
                "default",
                "No hay información suficiente "
                        + "para calcular la trazabilidad.",
                0L,
                0L,
                null,
                null,
    ...
    private void addUserHistoryPdfHeader(
        ...
        Font badgeFont = FontFactory.getFont(
                FontFactory.HELVETICA_BOLD,
                10,
                Color.WHITE
        );
        ...
        Paragraph title = new Paragraph(
                "Historial de usuario - Ventanilla",
                titleFont
        );
        ...
        Paragraph subtitle = new Paragraph(
                "Reporte individual de visitas y solicitudes "
                        + "registradas en el módulo de Ventanilla.",
                subtitleFont
        );
        ...
        badgeCell.setHorizontalAlignment(
                Element.ALIGN_CENTER
        );
        badgeCell.setVerticalAlignment(
        ...
        addUserInfoCell(
                userTable,
                "Cédula",
                history.cedulaUsuario()
        );

        addUserInfoCell(
                userTable,
                "Nombre",
                safeText(history.nombreUsuario())
        );

        addUserInfoCell(
                userTable,
                "Teléfono",
                safeText(history.telefono())
        );

        addUserInfoCell(
                userTable,
                "Fecha de generación",
                formatPdfDate(LocalDate.now())
        );
    ...
    private void addUserHistoryPdfSummary(
        ...
        addSummaryCell(
                summaryTable,
                "Total visitas",
                String.valueOf(history.totalVisitas())
        );

        addSummaryCell(
                summaryTable,
                "Total solicitudes",
                String.valueOf(history.totalSolicitudes())
        );

        addSummaryCell(
                summaryTable,
                "Primera visita",
                formatPdfDate(history.primeraVisita())
        );

        addSummaryCell(
                summaryTable,
                "Última visita",
                formatPdfDate(history.ultimaVisita())
        );
    ...
    private void addUserHistoryPdfTable(
        ...
        Paragraph sectionTitle = new Paragraph(
                "Detalle de solicitudes registradas",
                sectionFont
        );
        ...
        if (
                history.solicitudes() == null
                        || history.solicitudes().isEmpty()
        ) {
            Font emptyFont = FontFactory.getFont(
                    FontFactory.HELVETICA,
                    10,
                    new Color(90, 90, 90)
            );
        ...
        for (VentanillaUserHistoryItemResponse item :
            ...
            addBodyCell(
                    table,
                    formatPdfDate(item.fecha())
            );

            addBodyCell(
                    table,
                    safeText(item.numeroVentanilla())
            );

            addBodyCell(
                    table,
                    safeText(item.solicitudNombre())
            );

            addBodyCell(
                    table,
                    safeText(item.categoriaNombre())
            );

            addBodyCell(
                    table,
                    safeText(item.estadoSolicitudNombre())
            );

            addBodyCell(
                    table,
                    safeText(item.barrioNombre())
                            + " / "
                            + safeText(item.comunaNombre())
            );

            addBodyCell(
                    table,
                    safeText(item.funcionarioUsername())
            );
            ...
            addBodyCell(
                    table,
                    safeText(item.motivoRepeticion())
            );
    ...
    private void addUserHistoryPdfFooter(
            Document document
    ) throws DocumentException {
        ...
        Paragraph footer = new Paragraph(
                "Documento generado automáticamente por AppSisben. "
                        + "La información corresponde a registros activos "
                        + "e inactivos del módulo de Ventanilla.",
                footerFont
        );
    ...
    private record DailyRequestValidation(
            List<VentanillaRegistro> dailyRequests,
            boolean duplicatedRequest
    ) {
    }
